...
function parseBranchName(ref) {
  if (!ref) {
    switch (noRefAction) {
      case "error":
        _actions_core__WEBPACK_IMPORTED_MODULE_0__.setFailed("Unable to get github.ref/GITHUB_REF");
        return;
      case "warn":
        _actions_core__WEBPACK_IMPORTED_MODULE_0__.warning("Unable to get github.ref/GITHUB_REF");
        break;
      case "continue":
        break;
      default:
        _actions_core__WEBPACK_IMPORTED_MODULE_0__.setFailed(`Invalid value for bevActionOnNoRef: ${noRefAction}`);
        return;
    }
  }
...
try {
  canOverwrite = _actions_core__WEBPACK_IMPORTED_MODULE_0__.getInput("bevOverwrite") === "true";
  noRefAction = _actions_core__WEBPACK_IMPORTED_MODULE_0__.getInput("bevActionOnNoRef");
  setEmptyVars = _actions_core__WEBPACK_IMPORTED_MODULE_0__.getInput("bevSetEmptyVars") === "true";
  ...
  const vars = parseEnvVarPossibilities(process.env).forEach(
    ([name, possibleValues]) => {
      ...
      if (!value) {
        if (setEmptyVars) {
          _actions_core__WEBPACK_IMPORTED_MODULE_0__.exportVariable(name, "");
          _actions_core__WEBPACK_IMPORTED_MODULE_0__.debug(`Exporting ${name} with an empty value`);
        }
      } else {
        _actions_core__WEBPACK_IMPORTED_MODULE_0__.exportVariable(name, value);
        _actions_core__WEBPACK_IMPORTED_MODULE_0__.debug(`Exporting ${name} with value ${value}`);
      }
    }
  );
} catch (e) {
